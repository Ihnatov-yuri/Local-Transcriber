...
**Status: 1.2.0.** Recording, file import, transcription (Parakeet by default, Omnilingual for Arabic, whisper.cpp and Gemma 4 as alternatives, plus a Super mode that runs two engines and vote-merges), diarization (sherpa-onnx OR Gemma prompt-based), auto-titling, inline segment editing, four post-processing presets (Summary / Context-aware rewrite / Clean / Translate-polish), custom vocabulary, tone styles, snippets, multi-select constrained-auto language picker (Arabic / Ukrainian / English / Dutch), live transcription (Nemotron 3.5 streaming with sub-second partials, or chunked Gemma/Whisper/Parakeet/Omnilingual), and a Settings page with Gemma 4 compute knobs (GPU/CPU, context window, CPU threads).
